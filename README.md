...
ccresmon は LevNas プラグインの共通規約 ([claudecode-plugins/docs/development-guide.md](https://github.com/LevNas/claudecode-plugins/blob/main/docs/development-guide.md)) に従います。ccresmon は SKILL.md を持たない Hook 型プラグインです。

| 配置 | 用途 | 読者 |
|------|------|------|
| `README.md` | プラグイン概要・導入手順 | 利用者 (人間) |
| `ccresmon.sh` | フック本体 | 実行時 |
| `hooks.json` | フック定義 | Claude Code |
| `tests/ccresmon.bats` | BATS テスト | CI/開発者 |
| `docs/sdd/` | SDD 設計ドキュメント (requirements/design/tasks) | 開発者 (人間) |

中央リンタは SKILL.md を持たないプラグインを正常扱いにします。PR送付前に実行して確認してください:

```bash
bash ~/src/github.com/LevNas/claudecode-plugins/scripts/lint-skills.sh ~/src/github.com/LevNas/ccresmon
```
